...
import { Check, ArrowRight, Mountain, Home, School, Hospital, Train, Building, Music, Hammer } from 'lucide-react';
...
// --- Data setup for this specific service page ---

const slug = 'noise-survey';
const service = SERVICES.find((s) => s.slug === slug);
const serviceImage = PlaceHolderImages.find((p) => p.id === `service-${slug}`);
const professionalAdviceImage = PlaceHolderImages.find(p => p.id === "home-professional-advice");
const roadImage = PlaceHolderImages.find(p => p.id === 'cta-road');
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative w-full h-[70vh] min-h-[500px] flex items-center justify-center text-center text-white">
        {/* Use the serviceImage for the background */}
        {serviceImage ? (
          <Image
      ...
      {/* --- ALL OTHER SECTIONS FROM THE NOISE SURVEY PAGE --- */}
          ...
          <div className="bg-foreground py-24 flex items-center justify-end">
            <div className="container px-4 md:px-6 text-right">
              <h2 className="text-4xl md:text-5xl font-headline font-semibold text-background">
                ...
                <span className="block"><span className="text-accent">We</span> Do</span>
              </h2>
            </div>
          </div>
      ...
      <section className="py-16 md:py-24 bg-card border-t">
        <div className="container px-4 md:px-6">
          <div className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-headline font-semibold text-primary">Noise Survey Services</h2>
          </div>
          <Accordion type="single" collapsible className="w-full max-w-4xl mx-auto">
          {surveyServices.map((serviceItem) => {
            const image = PlaceHolderImages.find(p => p.id === serviceItem.imageId);
            return (
              <AccordionItem key={serviceItem.category} value={serviceItem.category}>
                <AccordionTrigger className="text-xl font-semibold hover:no-underline text-left">
                  <div className="flex items-center w-full self-stretch">
                    {image && (
                        <div className="relative w-24 self-stretch mr-4">
                            <Image src={image.imageUrl} alt={image.description} data-ai-hint={image.imageHint} fill className="object-cover rounded-lg" />
                        </div>
                    )}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8">
              <div className="p-6 bg-card rounded-lg shadow-sm">
                <h3 className="text-xl font-semibold font-headline text-primary mb-3">Low Cost</h3>
                <p className="text-muted-foreground font-light">Our processes and systems are highly efficient ensuring speed, accuracy and of course keeping our fees low.</p>
              </div>
              <div className="p-6 bg-card rounded-lg shadow-sm">
